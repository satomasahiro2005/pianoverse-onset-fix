...
| | 発音遅延 mean | ばらつき σ | 最悪値 max |
|---|---|---|---|
| **before** | 9.29 ms | 9.01 ms | 114.3 ms |
| **after**  | **1.57 ms** | **0.25 ms** | **1.73 ms** |

→ **遅延を約6分の1に短縮し、ばらつきを 97% 削減**（全サンプルがプリロール1.5msへ収束）。
...
| `onset-sweep.ps1` | 多数サンプルの発音遅延を一括計測 → CSV |
| `repack.ps1` | onset検出＋ヘッドトリム＋IKMPAK書き戻し（`Repack-PakTrimmed`） |
| `assets/make_figures.py` | 実測CSVから研究図（PNG）を生成（matplotlib） |
| `onset.csv` / `onset_close1_{before,after}.csv` | 計測結果 |

```powershell
# 発音遅延の計測
. .\onset-sweep.ps1 -Paks @("...\Close 1\Close 1.pak") -RrFilter ''

# ヘッドトリムして別ファイルに書き戻し（原本は触らない）
. .\pak.ps1; . .\repack.ps1
Repack-PakTrimmed -InPak "...\Close 1\Close 1.pak" `
                  -OutPak "...\Close 1\Close 1.trim.pak" -PrerollMs 1.5 -AnchorDb -20
```
```bash
python assets/make_figures.py     # 図を再生成
```
...
- [x] YF3 Close 1 をトリム → before/after 実測（9.3→1.6ms, σ −97%）
...
→ ばらつきの本命は **音程間**。修正案＝`.pak` 内の各ノートに **トレンドへ寄せるゲイン**を適用（頭トリムと同じ枠組み。
ヘッドルームを確認し減衰中心、過補正を避けて自然なバラつきは残す）。

---

## 8. 既知の課題（セルフレビュー）

- **弱音・高音の切りすぎ**: −20dBアンカーは pppp音で発音点が遅く、`A5 v9` が 112ms トリム等（trim>20ms: 8/214）。
  `MaxTrimMs` 上限を入れて soft-attack を保護すべき。
- **切り口のフェード未実装**: トリム後の先頭が −40dB前後から瞬間立ち上がり＝微小クリック源。
  0.3–0.5ms のフェードインで解消（実装予定）。
- **Pianoverse 実機ロードは未検証**: フォーマット根拠（チェックサム無し・パス参照）で安全と論証済みだが、
